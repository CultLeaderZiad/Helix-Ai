...
export async function generateClientMonthlyReport(
  ...
  const operationalBreakdown: OperationalMetric[] = [
    ...
    {
      system: 'Review queue',
      systemAr: 'قائمة المراجعة',
      metric: 'Items checked by a person',
  ...
  return {
    ...
    factAccuracyRate,
    systemUptimePercentage: 'n/a',
    executiveSummary: `During this monthly retainer cycle, Helix AI systems handled ${callsHandled} voice calls and ${whatsAppMessages} WhatsApp interactions for ${businessName}.`,
    executiveSummaryAr: `خلال دورة الاشتراك الشهري الحالية، عالج نظام Helix ${callsHandled} مكالمة صوتية و${whatsAppMessages} رسالة واتساب لصالح ${businessName}.`,
