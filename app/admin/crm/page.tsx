  ...
  return (
    <ConsoleShell variant="admin" email={session.user.email ?? ''} businessName={null}>
      <WholeCrmView
        ...
        pipelineValueCents={realDeals.reduce((sum, d) => sum + (d.value_cents ?? 0), 0)}
        verifiedFactCount={realFacts.filter(f => f.evidence_band === 'verified').length}
        totalFactCount={realFacts.length}
      />
    </ConsoleShell>
  )
}
